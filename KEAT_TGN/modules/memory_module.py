...
from modules.time_enc import TimeEncoder, TimeEncoderGM, CombinedLeTE
...
class TGNMemory(torch.nn.Module):
    def __init__(
        ...
        if time_enc_method == 'TGN':
            self.time_enc = TimeEncoder(time_dim)
        elif time_enc_method == 'GM':
            self.time_enc = TimeEncoderGM(time_dim)
        elif time_enc_method == 'LETE':
            self.time_enc = CombinedLeTE(time_dim)
        else:
            print('ERROR: Define valid time encoder')
    ...
    def device(self) -> torch.device:
        if self.time_enc_method in ['TGN', 'GM']:
            return self.time_enc.lin.weight.device
        else:
            return self.time_enc.w1_fourier.weight.device
