    ...
    def query_batch(self, 
        ...
        if (edge_type is None):
            ...
            if(self.dataset_name == 'tgbl-wiki'):
                num_neg = 999
            if(self.dataset_name == 'tgbl-review'):
                num_neg = 100
            for pos_s, pos_d, pos_t in zip(pos_src, pos_dst, pos_timestamp):
                if (pos_s, pos_d, pos_t) not in self.eval_set[split_mode]:
                    ...
                    x = [ int(neg_dst)  for neg_dst in self.eval_set[split_mode][(pos_s, pos_d, pos_t)] ]
                    if(len(x)<num_neg) : x=x+[min(int(pos_s.item()),int(pos_d.item()))+1]*(num_neg-len(x))
                    neg_samples.append(
                        x
                    )
        else:
