...
if __name__ == "__main__":
    ...
    if args.dataset_name == "tgbl-wiki":
        val_idx_data_loader = get_idx_data_loader(indices_list=list(range(len(val_data.src_node_ids))), batch_size=32, shuffle=False)
        test_idx_data_loader = get_idx_data_loader(indices_list=list(range(len(test_data.src_node_ids))), batch_size=32, shuffle=False)
    else:
    ...
    for run in range(args.num_runs):
        ...
        args.seed = run
        args.save_model_name = f'{args.model_name}_seed{args.seed}' + '_' + f'{args.model_version}' + 'keat'
        ...
        if args.model_name == 'TGAT':
            ...
            dynamic_backbone = DyGFormer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                         ...
                                         max_input_sequence_length=args.max_input_sequence_length, device=args.device, dataset_name=args.dataset_name)
        else:
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    # two Tensors, with shape (batch_size, output_dim)
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings_keat(src_node_ids=batch_neg_src_node_ids,
                                                                          dst_node_ids=batch_neg_dst_node_ids,
                                                                          node_interact_times=batch_node_interact_times)
                else:
                    raise ValueError(f"Wrong value for model_name {args.model_name}!")
