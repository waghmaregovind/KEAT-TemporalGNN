...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.transformers_keat = nn.ModuleList([
            TransformerEncoder_keat(attention_dim=self.num_channels * self.channel_embedding_dim, num_heads=self.num_heads, dropout=self.dropout)
    ...
    def compute_src_dst_node_temporal_embeddings_keat(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        """
        compute source and destination node temporal embeddings
        :param src_node_ids: ndarray, shape (batch_size, )
        :param dst_node_ids: ndarray, shape (batch_size, )
        ...
        src_padded_nodes_neighbor_ids, src_padded_nodes_edge_ids, src_padded_nodes_neighbor_times = \
            self.pad_sequences(node_ids=src_node_ids, node_interact_times=node_interact_times, nodes_neighbor_ids_list=src_nodes_neighbor_ids_list,
                               nodes_edge_ids_list=src_nodes_edge_ids_list, nodes_neighbor_times_list=src_nodes_neighbor_times_list,
                               patch_size=self.patch_size, max_input_sequence_length=self.max_input_sequence_length)
        batch_size = src_padded_nodes_neighbor_times.shape[0]
        src_num_patches = src_padded_nodes_neighbor_times.shape[1] // self.patch_size
        src_padded_nodes_neighbor_times_patches = src_padded_nodes_neighbor_times.reshape(batch_size, src_num_patches, self.patch_size)
        mask = (src_padded_nodes_neighbor_times_patches != -1.0)
        sums = np.sum(src_padded_nodes_neighbor_times_patches * mask, axis=2)
        src_num_times_per_patches = np.sum(mask, axis=2)
        src_num_times_per_patches = np.where(src_num_times_per_patches == 0, 1, src_num_times_per_patches)
        src_num_times_average = sums / src_num_times_per_patches
        src_num_times_average = src_num_times_average[..., np.newaxis]
        ...
        batch_size = dst_padded_nodes_neighbor_times.shape[0]
        dst_num_patches = dst_padded_nodes_neighbor_times.shape[1] // self.patch_size
        dst_padded_nodes_neighbor_times_patches = dst_padded_nodes_neighbor_times.reshape(batch_size, dst_num_patches, self.patch_size)
        mask = (dst_padded_nodes_neighbor_times_patches != -1.0)
        sums = np.sum(dst_padded_nodes_neighbor_times_patches * mask, axis=2)
        dst_num_times_per_patches = np.sum(mask, axis=2)
        dst_num_times_per_patches = np.where(dst_num_times_per_patches == 0, 1, dst_num_times_per_patches)
        dst_num_times_average = sums / dst_num_times_per_patches
        dst_num_times_average = dst_num_times_average[..., np.newaxis]
        ...
        src_patches_nodes_neighbor_time_features = self.projection_layer['time'](src_patches_nodes_neighbor_time_features)
        src_patches_nodes_neighbor_co_occurrence_features = self.projection_layer['neighbor_co_occurrence'](src_patches_nodes_neighbor_co_occurrence_features)
        src_patches_nodes_edge_raw_features1 = src_num_times_average * src_patches_nodes_edge_raw_features
        src_patches_nodes_neighbor_time_features1 = src_num_times_average * src_patches_nodes_neighbor_time_features
        ...
        dst_patches_nodes_neighbor_time_features = self.projection_layer['time'](dst_patches_nodes_neighbor_time_features)
        dst_patches_nodes_neighbor_co_occurrence_features = self.projection_layer['neighbor_co_occurrence'](dst_patches_nodes_neighbor_co_occurrence_features)
        dst_patches_nodes_edge_raw_features1 = dst_num_times_average * dst_patches_nodes_edge_raw_features
        dst_patches_nodes_neighbor_time_features1 = dst_num_times_average * dst_patches_nodes_neighbor_time_features
        ...
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, channel_embedding_dim)
        patches_nodes_neighbor_node_raw_features = torch.cat([src_patches_nodes_neighbor_node_raw_features, dst_patches_nodes_neighbor_node_raw_features], dim=1)
        patches_nodes_edge_raw_features = torch.cat([src_patches_nodes_edge_raw_features1, dst_patches_nodes_edge_raw_features1], dim=1)
        patches_nodes_neighbor_time_features = torch.cat([src_patches_nodes_neighbor_time_features1, dst_patches_nodes_neighbor_time_features1], dim=1)
        patches_nodes_neighbor_co_occurrence_features = torch.cat([src_patches_nodes_neighbor_co_occurrence_features, dst_patches_nodes_neighbor_co_occurrence_features], dim=1)

        patches_data1 = [patches_nodes_neighbor_node_raw_features, patches_nodes_edge_raw_features,
                        patches_nodes_neighbor_time_features, patches_nodes_neighbor_co_occurrence_features]
        # patches_data = [patches_nodes_neighbor_node_raw_features, patches_nodes_edge_raw_features,
        #                 patches_nodes_neighbor_co_occurrence_features]
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels, channel_embedding_dim)
        patches_data1 = torch.stack(patches_data1, dim=2)
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim)
        patches_data1 = patches_data1.reshape(batch_size, src_num_patches + dst_num_patches, self.num_channels * self.channel_embedding_dim).float()


        src_patches_nodes_edge_raw_features2 = src_num_times_average_ * src_patches_nodes_edge_raw_features
        src_patches_nodes_neighbor_time_features2 = src_num_times_average_ * src_patches_nodes_neighbor_time_features
        
        # Tensor, shape (batch_size, dst_num_patches, channel_embedding_dim)
        
        dst_patches_nodes_edge_raw_features2 = dst_num_times_average_ * dst_patches_nodes_edge_raw_features
        dst_patches_nodes_neighbor_time_features2 = dst_num_times_average_ * dst_patches_nodes_neighbor_time_features
        ...
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, channel_embedding_dim)
        patches_nodes_neighbor_node_raw_features = torch.cat([src_patches_nodes_neighbor_node_raw_features, dst_patches_nodes_neighbor_node_raw_features], dim=1)
        patches_nodes_edge_raw_features = torch.cat([src_patches_nodes_edge_raw_features2, dst_patches_nodes_edge_raw_features2], dim=1)
        patches_nodes_neighbor_time_features = torch.cat([src_patches_nodes_neighbor_time_features2, dst_patches_nodes_neighbor_time_features2], dim=1)
        patches_nodes_neighbor_co_occurrence_features = torch.cat([src_patches_nodes_neighbor_co_occurrence_features, dst_patches_nodes_neighbor_co_occurrence_features], dim=1)

        patches_data2 = [patches_nodes_neighbor_node_raw_features, patches_nodes_edge_raw_features,
                        patches_nodes_neighbor_time_features, patches_nodes_neighbor_co_occurrence_features]
        # patches_data = [patches_nodes_neighbor_node_raw_features, patches_nodes_edge_raw_features,
        #                 patches_nodes_neighbor_co_occurrence_features]
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels, channel_embedding_dim)
        patches_data2 = torch.stack(patches_data2, dim=2)
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim)
        patches_data2 = patches_data2.reshape(batch_size, src_num_patches + dst_num_patches, self.num_channels * self.channel_embedding_dim).float()
        ...
        patches_data = torch.stack(patches_data, dim=2)
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim)
        patches_data = patches_data.reshape(batch_size, src_num_patches + dst_num_patches, self.num_channels * self.channel_embedding_dim).float()
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim)
        for transformer in self.transformers_keat:
            patches_data = transformer(patches_data1,patches_data2,patches_data)
    ...
    def pad_sequences(self, node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: list, nodes_edge_ids_list: list,
        ...
        padded_nodes_neighbor_times = np.full((len(node_ids), max_seq_length),-1).astype(np.float32)
...
class TransformerEncoder_keat(nn.Module):
    ...
    def __init__(self, attention_dim: int, num_heads: int, dropout: float = 0.1):
        ...
        :param dropout: float, dropout rate
        """
        super(TransformerEncoder_keat, self).__init__()
        # use the MultiheadAttention implemented by PyTorch
        self.multi_head_attention = MultiheadAttention(embed_dim=attention_dim, num_heads=num_heads, dropout=dropout)
    ...
    def forward(self, input1: torch.Tensor,input2: torch.Tensor,input3: torch.Tensor):
        ...
        # note that the MultiheadAttention module accept input data with shape (seq_length, batch_size, input_dim), so we need to transpose the input
        # Tensor, shape (num_patches, batch_size, self.attention_dim)
        transposed_input1 = input1.transpose(0, 1)
        transposed_input2 = input2.transpose(0, 1)
        transposed_input3 = input3.transpose(0, 1)
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        transposed_input1 = self.norm_layers[0](transposed_input1)
        transposed_input2 = self.norm_layers[0](transposed_input2)
        transposed_input3 = self.norm_layers[0](transposed_input3)
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        hidden_states = self.multi_head_attention(query=transposed_input1, key=transposed_input2, value=transposed_input3)[0].transpose(0, 1)
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        outputs = input3 + self.dropout(hidden_states)
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        hidden_states = self.linear_layers[1](self.dropout(F.gelu(self.linear_layers[0](self.norm_layers[1](outputs)))))
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        outputs = outputs + self.dropout(hidden_states)
