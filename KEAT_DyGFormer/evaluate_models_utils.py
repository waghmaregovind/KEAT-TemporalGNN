...
from utils.negative_sampler import NegativeEdgeSampler
...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   eval_metric_name: str, evaluator: LinkPredictionEvaluator, num_neighbors: int = 20, time_gap: int = 2000, mode: str = 'hist_rnd',data_std: int =None):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            num_negative_samples_per_node = 0
            if (mode == "hist_rnd") :
            ...
            # we should pay attention to the mappings of node ids, add 1 to convert to the mapped node ids in our implementation
                ...
                batch_neg_dst_node_ids_list = np.array(batch_neg_dst_node_ids_list)
                if(not len(batch_neg_dst_node_ids_list.shape)==1):
                    # batch_neg_dst_node_ids_list = np.expand_dims(batch_neg_dst_node_ids_list[:,0], axis=1)
                    batch_neg_dst_node_ids = batch_neg_dst_node_ids_list + 1
            ...
            # we should pay attention to the mappings of node ids, add 1 to convert to the mapped node ids in our implementation
                ...
                num_negative_samples_per_node = 3
                batch_neg_dst_node_ids = np.repeat(batch_dst_node_ids, repeats=num_negative_samples_per_node, axis=0)   
                neg_times = []
                for std_multiplier in range(1,4):
                     neg_times.append(batch_node_interact_times + std_multiplier*data_std)           
                repeated_batch_node_interact_times = np.concatenate(neg_times ,axis=0)


            elif (mode == "1vs5_std"):

                num_negative_samples_per_node = 5
                batch_neg_dst_node_ids = np.repeat(batch_dst_node_ids, repeats=num_negative_samples_per_node, axis=0)   
                neg_times = []
                for std_multiplier in range(1,6):
                     neg_times.append(batch_node_interact_times + std_multiplier*data_std)           
                repeated_batch_node_interact_times = np.concatenate(neg_times ,axis=0)

            elif (mode == "1vs7_std"):

                num_negative_samples_per_node = 7
                batch_neg_dst_node_ids = np.repeat(batch_dst_node_ids, repeats=num_negative_samples_per_node, axis=0)   
                neg_times = []
                for std_multiplier in range(1,8):
                     neg_times.append(batch_node_interact_times + std_multiplier*data_std)           
                repeated_batch_node_interact_times = np.concatenate(neg_times, axis=0)
            
            elif (mode == "1vs10_std"):

                num_negative_samples_per_node = 10
                batch_neg_dst_node_ids = np.repeat(batch_dst_node_ids, repeats=num_negative_samples_per_node, axis=0)   
                neg_times = []
                for std_multiplier in range(1,11):
                     neg_times.append(batch_node_interact_times + std_multiplier*data_std)           
                repeated_batch_node_interact_times = np.concatenate(neg_times, axis=0)
            
            elif (mode == "1vs25_std"):

                num_negative_samples_per_node = 25
                batch_neg_dst_node_ids = np.repeat(batch_dst_node_ids, repeats=num_negative_samples_per_node, axis=0)   
                neg_times = []
                for std_multiplier in range(1,26):
                     neg_times.append(batch_node_interact_times + std_multiplier*data_std)           
                repeated_batch_node_interact_times = np.concatenate(neg_times, axis=0)


            else:
                num_negative_samples_per_node = 1
                batch_neg_dst_node_ids = batch_dst_node_ids               
                repeated_batch_node_interact_times = np.repeat(batch_node_interact_times + data_std, repeats=num_negative_samples_per_node, axis=0)
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                #                                                       node_interact_times=repeated_batch_node_interact_times)
                src_node_embeddings, dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings_keat(src_node_ids=batch_src_node_ids,
                                                                      dst_node_ids=batch_dst_node_ids,
                                                                      node_interact_times=batch_node_interact_times)
                ...
                # two Tensors, with shape (batch_size * num_negative_samples_per_node, output_dim)
                neg_src_node_embeddings, neg_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings_keat(src_node_ids=repeated_batch_src_node_ids,
                                                                      dst_node_ids=batch_neg_dst_node_ids.flatten(),
                                                                      node_interact_times=repeated_batch_node_interact_times)
